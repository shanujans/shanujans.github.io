...
const badges = [
  { icon: 'fas fa-cloud',     title: 'AI in Action',    issuer: 'Google Cloud',  date: 'Mar 2026', color: '#4285F4', sub: 'Cloud AI Series' },
  { icon: 'fas fa-atom',      title: 'Quantum Info',    issuer: 'IBM',           date: 'Apr 2025', color: '#00ff9d', sub: 'Basics of Quantum' },
  { icon: 'fas fa-brain',     title: 'AI Fundamentals', issuer: 'IBM SkillsBuild', date: 'Apr 2025', color: '#7700ff', sub: 'AI Fundamentals' },
  { icon: 'fas fa-chart-bar', title: 'Data Analytics',  issuer: 'Cisco',         date: 'Mar 2025', color: '#00b3ff', sub: 'Analytics Essentials' },
];
...
const Experience: React.FC = () => {
  ...
  useGSAP(() => {
    ...
    if (badgesRef.current) {
      gsap.fromTo(
        ...
        { opacity: 0, y: 24, scale: 0.9 },
        {
          ...
          scrollTrigger: { trigger: badgesRef.current, start: 'top 82%' },
        }
      );
    }
    ...
    if (lineRef.current) {
      gsap.fromTo(lineRef.current,
        { scaleY: 0, transformOrigin: 'top' },
        {
          ...
          scrollTrigger: { trigger: timelineRef.current, start: 'top 80%', end: 'bottom 60%', scrub: 0.5 },
        }
      );
    }

    // Timeline items — sequential reveal as they enter viewport
    if (timelineRef.current) {
      ...
      items.forEach((item) => {
        ...
        gsap.fromTo(item,
          { opacity: 0, x: isLeft ? -40 : 40, filter: 'blur(4px)' },
          {
  ...
  return (
    <section ref={sectionRef} id="experience" className="py-24 scroll-mt-20 bg-black/20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto mb-16">
          ...
          <div ref={badgesRef} className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {badges.map(b => (
              <div
                key={b.title}
                className="flex flex-col items-center p-4 rounded-xl bg-white/5 backdrop-blur-sm border hover:-translate-y-1 transition-all duration-300"
                style={{ borderColor: b.color + '30', opacity: 0 }}
                onMouseEnter={e => {
        ...
        <div ref={timelineRef} className="relative max-w-4xl mx-auto">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(to bottom, transparent, rgba(0,255,157,0.3) 20%, rgba(0,255,157,0.3) 80%, transparent)',
              transformOrigin: 'top',
            }}
          />
          ...
          {experience.map((item, index) => {
            ...
            const border  = item.type === 'work' ? 'border-[#00ff9d]/30' : 'border-[#7700ff]/30';
            const badge   = item.type === 'work' ? 'bg-[#00ff9d]/10 text-[#00ff9d]' : 'bg-[#7700ff]/10 text-[#7700ff]';

            return (
              <div
                key={index}
                className={`timeline-item ${isLeft ? 'timeline-left' : 'timeline-right'} relative flex items-start gap-0 md:gap-8 mb-12 ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'}`}
                style={{ opacity: 0 }}
              >
                {/* Card */}
                <div className={`flex-1 ${isLeft ? 'md:text-right md:pr-8' : 'md:text-left md:pl-8'} pl-12 md:pl-0`}>
                  <div
                    className={`p-6 rounded-xl border ${border} bg-white/5 backdrop-blur-sm hover:-translate-y-1 transition-all duration-300 group cursor-default`}
                    onMouseEnter={e => {
                      (e.currentTarget as HTMLElement).style.boxShadow = `0 16px 40px ${color}18`;
                  ...
                  >
                    <div className={`flex items-center gap-3 mb-3 ${isLeft ? 'md:justify-end' : 'md:justify-start'} flex-wrap`}>
                      <span className={`text-xs font-jetbrains-mono px-2 py-1 rounded ${badge}`}>
                ...
                <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 top-6 z-10">
                  <div
                    ...
                    style={{ borderColor: color, background: '#0a0a14', boxShadow: `0 0 20px ${color}50` }}
                  >
